...
export function multiProductTradePrompt(message, symbols) {
  return `Parse one Binance trade request into JSON. Products: spot, margin, futures. You may also return an asset operation: {"action":"TRANSFER","asset":"USDT","amount":"10","type":"MAIN_UMFUTURE|MAIN_CMFUTURE|UMFUTURE_MAIN|CMFUTURE_MAIN"} or {"action":"CONVERT","asset":"BTC","toAsset":"USDT","amount":"0.001"}. Never invent missing values. Supported symbols: ${symbols.join(', ')}. Futures requires symbol, BUY/SELL, entryType MARKET or LIMIT, quantity, integer leverage 1-125, and marginType ISOLATED or CROSSED. Optional futures protection: stopLoss number and takeProfits array of positive numbers; if an entry range is supplied use entryPrice or entryLow/entryHigh. For a LONG/BUY, stopLoss must be below entry and takeProfits above entry; reverse for SELL. Spot uses the normal MARKET/LIMIT schema. Set directExecute true only when the user explicitly says 立即执行、直接执行、无需确认、不要确认 or equivalent; otherwise false. Return JSON only: {"reply":"...","product":"spot|margin|futures","action":null,"intent":null,"directExecute":false} when incomplete, otherwise the same object with intent or action. Never claim an order was executed. User message: ${JSON.stringify(String(message).slice(0, 3000))}`;
}
