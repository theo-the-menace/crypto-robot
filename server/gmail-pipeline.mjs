...
export async function analyzeGmailMessage(message) {
  if (!baseUrl || !apiKey) throw new Error('Configure GMAIL_LLM_BASE_URL/GMAIL_LLM_API_KEY or the gateway credentials.');
  const response = await fetch(`${baseUrl}/v1/chat/completions`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model, reasoning_effort: 'medium', messages: [{ role: 'system', content: prompt(message) }], temperature: 0 }), signal: AbortSignal.timeout(45_000) });
  const body = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(body.error?.message || `LLM gateway failed (${response.status}).`);
  const raw = String(body.choices?.[0]?.message?.content || '').replace(/^```json\s*|\s*```$/g, '').trim();
...
export async function processGmailMessage(message, store) {
  ...
  try {
    ...
    return { message: await store.add({ source: 'CME Group', sourceMessageId: message.id, title: analysis.title || message.subject || 'CME Group market update', summary: analysis.summary || '', analysis: analysis.analysis || '', impact: analysis.impact || 'neutral', confidence: Number(analysis.confidence) || 0, chart: analysis.chart || null, original: { from: message.from, subject: message.subject, text: message.text, internalDate: message.internalDate } }) };
