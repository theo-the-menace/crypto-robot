...
export async function completeWithOverseasStrategy({ content, model: requestedModel } = {}) {
  ...
  for (const route of routes) {
    const base = value(route.base); const key = value(route.key); const model = requestedModel || modelList(route.models)[0];
