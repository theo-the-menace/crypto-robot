...
const cacheFile = resolve(root, 'whitehouse.json');
const pages = ['https://www.whitehouse.gov/remarks/', 'https://www.whitehouse.gov/briefing-room/', 'https://www.whitehouse.gov/presidential-actions/', 'https://www.whitehouse.gov/videos/'];
...
const linksFrom = (html) => [...String(html).matchAll(/href=["'](https:\/\/www\.whitehouse\.gov\/(?:remarks|releases|briefing-room|presidential-actions|videos)\/[^"'?#]+\/?)["']/gi)].map((match) => match[1]);
...
export async function collectWhiteHouse({ onRelevant } = {}) {
  ...
  for (const page of pages) {
    ...
    for (const url of [...new Set(linksFrom(html))].slice(0, 20)) {
      if (processed >= 20) break;
      if (cache[url]) continue;
      discovered += 1;
      try {
        processed += 1;
        const detail = await fetchPage(url); const title = clean(meta(detail, 'og:title')) || clean(detail.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1]) || url; const content = clean(detail); const item = { id: url, source: 'White House', sourceUrl: url, title, content, excerpt: content.slice(0, 2_500), publishedAt: Date.now(), collectedAt: Date.now(), impactScore: await score({ title, excerpt: content.slice(0, 2_500) }) };
